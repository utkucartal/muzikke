...
module.exports = {
  ...
  let m = await message.channel.send(":white_check_mark:  Şarkı sözlerini arıyorum.")  
...
.then(results => {
    ...
    .then(lyrics => {
      if (lyrics.length > 9999) {
        return message.channel.send(":x: Şarkı sözleri çoook uzun.")
      }
...
}).catch(err => message.channel.send(":x: Şarkı sözlerini bulamadım."));
