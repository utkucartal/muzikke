...
module.exports = {
  ...
  async execute(client, message, args) {
    ...
    if (!args.length) {
      ...
      return message.channel.send(":x: Şarkı ismi veya Linki Vermen Gerek");
    }
    ...
    if (!videoPattern.test(args[0]) && playlistPattern.test(args[0])) {
      return message.channel.send(":x: Oynatma listesi oynatılamıyor.");
    }
    ...
    if(!serverQueue) message.client.queue.set(message.guild.id, queueConstruct)
     ...
     if (!serverQueue) {
      try {
        ...
        await channel.leave();
        return message.channel.send({embed: {"description": `:x: Kanala giriş yapamıyorum.: ${error}`, "color": "#ff2050"}}).catch(console.error);
      }
    }
